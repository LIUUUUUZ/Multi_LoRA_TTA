...
# Mixed severity: for each corruption type, adapt on severity 1-5, evaluate on severity=5
# Set to "--mixed_severity" to enable, or "" to disable
MIXED_SEVERITY=""
            ...
            for memory_type in "HUS"; do #
              for TGT in $TGTS; do
                python main.py --gpu_idx ${GPUS[i % ${NUM_GPUS}]} --dataset $DATASET --method SoTTA --tgt ${TGT} --model $MODEL --epoch $EPOCH ${CP} --seed $SEED \
                  --remove_cp --online --use_learned_stats --lr ${lr} --weight_decay ${weight_decay} --tgt_train_dist ${dist} \
                  --update_every_x ${update_every_x} --memory_size ${memory_size} --memory_type ${memory_type} --bn_momentum ${bn_momentum} \
                  --log_prefix ${LOG_PREFIX}${MIXED_SUFFIX}_${SEED}_dist${dist}_mt${bn_momentum}_${memory_type}_ht${high_threshold}_lr${lr} \
            ...
            for memory_type in "CSTU"; do #
              for TGT in $TGTS; do
                python main.py --gpu_idx ${GPUS[i % ${NUM_GPUS}]} --dataset $DATASET --method "RoTTA" --tgt ${TGT} --model $MODEL --epoch $EPOCH ${CP} --seed $SEED \
                  --remove_cp --online --use_learned_stats --lr ${lr} --weight_decay ${weight_decay} --tgt_train_dist ${dist} \
                  --update_every_x ${update_every_x} --memory_size ${memory_size} --memory_type ${memory_type} --bn_momentum "0.05" \
                  --log_prefix "${LOG_PREFIX}${MIXED_SUFFIX}_${SEED}_dist${dist}_mt0.05_${memory_type}" \
                  --loss_scaler ${loss_scaler} \
                  ${NOISY_TYPE} \
                  ${validation} \
                  ${MIXED_SEVERITY} ${MIXED_CORRUPTION_SEVERITY} \
...
# mixed_corruption_severity runs its own dedicated section below; skip the per-TGT loop
if [ -z "${MIXED_CORRUPTION_SEVERITY}" ]; then
  test_time_adaptation
fi

# ─────────────────────────────────────────────────────────────────────────────
# mixed_corruption_severity: run ONCE with all corruption types × all severities
# Only executed when MIXED_CORRUPTION_SEVERITY is set.
# ─────────────────────────────────────────────────────────────────────────────
if [ -n "${MIXED_CORRUPTION_SEVERITY}" ]; then
