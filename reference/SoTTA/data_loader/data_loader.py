...
def domain_data_loader(dataset, domains, file_path, batch_size, train_max_rows=np.inf, valid_max_rows=np.inf,
    ...
    if dataset in ['imagenetnoisy', 'cifar10noisy', 'cifar100noisy']:

        cond = processed_domains
        transform = 'src' if is_src else 'val'
        if not is_src and isinstance(processed_domains, list) and len(processed_domains) > 1:
            # multi-domain (mixed_severity or mixed_corruption_severity): one NoisyDataset per domain
            # domain_label encoding:
            #   mixed_severity:             domain_label = severity (1-5)
            #   mixed_corruption_severity:  domain_label = (corruption_idx+1)*10 + severity
            #     -> severity = domain_label % 10,  corruption_idx = domain_label // 10 - 1
            from conf import CORRUPTION_LIST
            for dom in processed_domains:
                parts = dom.rsplit('-', 1)
                sev_num = int(parts[1]) if len(parts) == 2 and parts[1].isdigit() else 0
                corr_name = parts[0]
                if getattr(conf.args, 'mixed_corruption_severity', False) and corr_name in CORRUPTION_LIST:
                    corr_idx = CORRUPTION_LIST.index(corr_name)
                    domain_lbl = (corr_idx + 1) * 10 + sev_num
                else:
                    domain_lbl = sev_num
                loaded_data = NoisyDataset(base=dataset, domains=[dom], transform=transform,
                                           noisy_data=conf.args.noisy_type,
                                           noisy_data_size=conf.args.noisy_size,
                                           noisy_data_class=conf.args.noisy_class,
                                           severity_label=domain_lbl)
                entire_datasets.append(loaded_data)
        else:
            loaded_data = NoisyDataset(base=dataset, domains=cond, transform=transform,
        ...
        if not loaded_data:
            loaded_data = CIFAR10Dataset(domains=cond, transform=transform)
        ...
        if not loaded_data:
            loaded_data = CIFAR100Dataset(domains=cond, transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)
        ...
        if not loaded_data:
            loaded_data = ImageNetDataset(domain=cond[0], transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)
