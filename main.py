...
def main():
    ...
    if conf.args.mixed_corruption_severity:
        # Load ALL corruption types × ALL severity levels as one combined stream
        # Takes priority over mixed_severity when both flags are set
        corruption_names = [d.rsplit('-', 1)[0] for d in conf.args.opt['tgt_domains']]
        tgt_for_loading = [f"{c}-{s}" for c in corruption_names for s in range(1, 6)]
        print(f'Mixed corruption+severity enabled: loading {len(corruption_names)} corruptions × 5 severities')
        
    elif conf.args.mixed_severity and conf.args.tgt:
        parts = conf.args.tgt.rsplit('-', 1)
        if len(parts) == 2 and parts[1].isdigit():
            tgt_for_loading = [f"{parts[0]}-{s}" for s in range(1, 6)]
            print(f'Mixed severity enabled: loading severities 1-5 for {parts[0]}')
        else:
            tgt_for_loading = conf.args.tgt
    else:
        tgt_for_loading = conf.args.tgt

    target_data_loader = data_loader.domain_data_loader(conf.args.dataset, tgt_for_loading,
                                                        conf.args.opt['file_path'],
                                                        batch_size=conf.args.opt['batch_size'],
                                                        valid_split=0,
    ...
    if not conf.args.online:
        ...
        print(f"num of samples {num_sample_end}")
        best_acc = -9999
        best_epoch = -1
...
def parse_arguments():
    ...
    parser.add_argument('--log_grad', action='store_true', default=False,
                        help="log grad-norm")
    parser.add_argument('--mixed_severity', action='store_true', default=False,
                        help='For a single corruption type, use mixed severity 1-5 as TTA stream; '
                             'online eval only counts severity=5 samples')
