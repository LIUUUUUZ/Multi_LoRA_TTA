...
class DNN:
    ...
    def target_data_cache(self):
        ...
        filename = f"{dataset}_{conf.args.noisy_type}_{conf.args.noisy_size}_{conf.args.noisy_class}_" \
                   ...
                   f"_ms{conf.args.mixed_severity}_mcs{conf.args.mixed_corruption_severity}"
    ...
    def log_loss_results(self, condition, epoch, loss_avg):
        ...
        if condition == 'train_online':
            ...
            # total = len(self.target_train_set[0]) if hasattr(self, 'target_train_set') else 0
            # interval = max(1, total // 1000)
            # if total == 0 or epoch % interval == 0:
    ...
    def evaluation_online_body(self, epoch, current_samples, feats, cls, dls):
        ...
        try:
            ...
            per_corruption_gt = self.json.get('per_corruption_gt', {})
            per_corruption_pred = self.json.get('per_corruption_pred', {})
        except KeyError:
            true_cls_list = []
            pred_cls_list = []
            accuracy_list = []
        ...
        in_domain_mask = cls.cpu() != NOISY_CLASS_IDX
        if conf.args.mixed_corruption_severity:
            # domain_label = (corruption_idx+1)*10 + severity
            # severity=5 in-domain: domain_label % 10 == 5 and domain_label != 0
            dls_cpu = dls.cpu()
            sev5_mask = in_domain_mask & (dls_cpu % 10 == 5) & (dls_cpu != 0)
            true_cls_list += [int(c) for c, m in zip(cls.cpu(), sev5_mask) if m]
            dls_in = dls_cpu[in_domain_mask]
            sev5_in_mask = (dls_in % 10 == 5) & (dls_in != 0)
            pred_cls_list += [int(c) for c, m in zip(y_pred.cpu().tolist(), sev5_in_mask) if m]

            # per-corruption tracking (severity=5 in-domain only)
            from conf import CORRUPTION_LIST
            for gt_c, pr_c, dl_v in zip(cls.cpu().tolist(), y_pred.cpu().tolist(), dls_cpu.tolist()):
                dl_v = int(dl_v)
                if gt_c == NOISY_CLASS_IDX or dl_v == 0:
                    continue
                if dl_v % 10 == 5:
                    corr_idx = dl_v // 10 - 1
                    if 0 <= corr_idx < len(CORRUPTION_LIST):
                        cname = CORRUPTION_LIST[corr_idx]
                        if cname not in per_corruption_gt:
                            per_corruption_gt[cname] = []
                            per_corruption_pred[cname] = []
                        per_corruption_gt[cname].append(int(gt_c))
                        per_corruption_pred[cname].append(int(pr_c))
        elif conf.args.mixed_severity:
            # only count severity=5 in-domain samples for accuracy (domain_label==5)
            sev5_mask = in_domain_mask & (dls.cpu() == 5)
            true_cls_list += [int(c) for c, m in zip(cls.cpu(), sev5_mask) if m]
            dls_in = dls.cpu()[in_domain_mask]
            pred_cls_list += [int(c) for c, m in zip(y_pred.cpu().tolist(), dls_in == 5) if m]
        else:
            true_cls_list += [int(c) for c in cls if c != NOISY_CLASS_IDX]
            pred_cls_list += [int(c) for c in y_pred.tolist() if c != NOISY_CLASS_IDX]
        conf_list += [float(c) for c in y_conf[in_domain_mask].tolist()]
        entropy_list += [float(c) for c in entropy[in_domain_mask].tolist()]
        energy_list += [float(c) for c in y_energy[in_domain_mask].tolist()]
        ...
        if len(true_cls_list) > 0:
            ...
            for i in range(epoch + 1 - len(current_samples[0]), epoch + 1):  # consider a batch input
                if i in progress_checkpoint:
                    if conf.args.mixed_corruption_severity:
                        per_corr_acc_str = ', '.join(
                            f'{cn}:{sum(g==p for g,p in zip(per_corruption_gt[cn], per_corruption_pred[cn]))/len(per_corruption_gt[cn])*100:.2f}'
                            for cn in per_corruption_gt if len(per_corruption_gt[cn]) > 0
                        )
                        print(
                            f'[Online Eval][NumSample:{i}][Epoch:{progress_checkpoint.index(i) + 1}]'
                            f'[Accuracy:{cumul_accuracy}][PerCorruption:{per_corr_acc_str}]')
                    else:
                        print(
        ...
        self.json = {
            ...
            'per_corruption_gt': per_corruption_gt,
            'per_corruption_pred': per_corruption_pred,
        }
    ...
    def dump_eval_online_result(self, is_train_offline=False):
        ...
        # compute and log per-corruption accuracy for mixed_corruption_severity
        if conf.args.mixed_corruption_severity:
            from conf import CORRUPTION_LIST
            per_corr_gt = self.json.get('per_corruption_gt', {})
            per_corr_pred = self.json.get('per_corruption_pred', {})
            per_corruption_accuracy = {}
            for cname in CORRUPTION_LIST:
                gts = per_corr_gt.get(cname, [])
                preds = per_corr_pred.get(cname, [])
                if len(gts) > 0:
                    acc = sum(g == p for g, p in zip(gts, preds)) / len(gts) * 100
                    per_corruption_accuracy[cname] = acc
                    print(f'[PerCorruption][{cname}][NumSamples:{len(gts)}][Accuracy:{acc:.4f}]')
                else:
                    per_corruption_accuracy[cname] = None
            self.json['per_corruption_accuracy'] = per_corruption_accuracy
    ...
    def eval_on_sev5(self, sev5_dataloader):
        """After mixed-corruption TTA, evaluate the adapted model on severity=5 data without
        further adaptation and write results to online_eval.json (same format as standard runs)."""
        features, cl_labels, do_labels = [], [], []
        for feat, cl, dl in sev5_dataloader['train']:
            features.append(feat.squeeze(0))
            cl_labels.append(cl.squeeze())
            do_labels.append(dl.squeeze())
        ...
        self.json = {}  # reset accumulated online-eval state from the mixed TTA phase

        print(f'[Severity-5 Final Eval] evaluating on {len(features)} samples...')
        self.dump_eval_online_result(is_train_offline=True)
